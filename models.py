...
class PECNN_dynamic_timefirst(BaseModel_dynamic):
    ...
    def __init__(self, c = 8):
        ...
        'batch normalization'
        # Encoderblock 1
        self.conv11 = nn.Conv3d(1+1, c, kernel_size=3, padding=1, padding_mode='reflect') # +1 for timedimension
        ...
        # Middle Block
        self.conv_m1 = nn.Conv3d(c*4, c*8, kernel_size=3, padding=1, padding_mode='reflect')      # here +c*4 inputlayer filled with the current timestep
...
class PECNN_dynamic(BaseModel_dynamic):
    ...
    def __init__(self, c = 8):
        ...
        # Middle Block
        self.conv_m1 = nn.Conv3d(c*4+ c*4, c*8, kernel_size=3, padding=1, padding_mode='reflect')      # here +c*4 inputlayer filled with the current timestep
        self.gn_m1 = nn.GroupNorm(num_groups=int(c), num_channels= c*8)                             # groups = int(c) because it would be c*8/8
        self.conv_m2 = nn.Conv3d(c*8, c*8, kernel_size=3, padding=1, padding_mode='reflect')
        self.gn_m2 = nn.GroupNorm(num_groups=int(c), num_channels=c*8)
    ...
    def forward(self,x_input, time):
        ...
        'normalize input time, e.g. /100'
        # Ensure time is reshaped to the correct shape
        time_outputdim = self.c*4    # enhance value of time_output dimensions such that the time has more impact, remember to change input channels of conv_m1
        time = time.view(batch, 1, 1, 1, 1)
        time_channel = time.expand(batch, time_outputdim, x_dim, y_dim, z_dim)
...
class PECNN_dynamic_batchnorm(PECNN_dynamic):
    class PECNN_dynamic_batchnorm(PECNN_dynamic):
        def __init__(self, loss_fn, c=8):
            super(PECNN_dynamic_batchnorm, self).__init__(loss_fn=loss_fn, c=c)

            # Replacing GroupNorm with BatchNorm3d
...
if __name__ == '__main__':
    ...
    model = PECNN_dynamic_timefirst(loss_fn = torch.nn.MSELoss()).to(device)
    #summary(model, (1, 1, 32, 64, 16))
    batch = 2
    x = torch.arange(batch*1*32*64*16).reshape(batch, 1, 64, 32, 16).to(device)
    t = .1
    t_tensor = torch.full((batch, 1), t)
    y = model(x.float(),t_tensor.float())
    print(y.shape)
